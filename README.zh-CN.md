...
**我们的起点是从论文独立重建方法与计算，而不止于阅读总结或封装已有科研代码。**
...
这些基础已在本仓库实现。自动生成的论文关系需要核验，面向非技术用户的易用体验也在持续改进。

### 三个项目，一条研究路径

CyberEinstein 建立在 PRAgent 已有的复现引擎和 RunThePaper 已积累的研究资产之上：
...
PRAgent 的独立重建不以作者发布可运行代码为前提；缺失的科学输入或实验资源会作为明确的复现边界保留。[RunThePaper](https://github.com/xi-zhao/RunThePaper) 已积累上百篇物理论文的复现案例，保留成功、受阻与失败的探索过程，案例库仍在持续扩充。现在即可阅读和运行其中的公开案例；PRAgent 开发仓的访问权限与本仓库的安装相互独立。

**下一步，是围绕一个真实的未解决问题，跑通一次能够独立检查的研究过程。** 将领域历史、复现证据与新问题跨越三个项目连接起来。它可能带来新结果，也可能排除一种解释。我们会如实保留证据与局限，让下一次尝试有更好的起点。
