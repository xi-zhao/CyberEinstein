...
[PRAgent](https://github.com/xi-zhao/PRAgent) 是我们的论文复现引擎：从论文重建推导与方法，独立编写并运行数值代码，对照论文结果，组织验证与独立审查。[RunThePaper](https://github.com/xi-zhao/RunThePaper) 已积累上百篇物理论文的复现案例，保存推导、代码、数据、图表、证据，以及成功、受阻与失败的探索过程，案例库仍在持续扩充。

**PRAgent 负责重建与检验，RunThePaper 负责保存与共享，CyberEinstein 在这些已有工作上继续探索原创研究。** 下一阶段，是将领域历史、复现证据与新的研究问题连接起来，形成跨越三个项目、能够持续推进的原创研究流程。
